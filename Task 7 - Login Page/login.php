...
if (!empty($_POST['email']) && !empty($_POST['password'])) {
    ...
    $qry = "select email,password from user where email ='$user->email'";
    $res = $DB->Select($qry);
    if (count($res) > 0) {
        $hashedPw = $res[0]['password'];
        if (password_verify($user->password, $hashedPw)) {
            session_start();
        ...
        $alertMsg = 'This email does not exist!';
    }
    $DB->CloseCon();
}
